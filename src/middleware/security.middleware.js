...
const securityMiddleware = async (req, res, next) => {
  // Bypass Arcjet in test environment to allow endpoint tests to pass
  ...
  try {
    ...
    if (decision.isDenied() && decision.reason.isBot()) {
      logger.warn('Request blocked as bot detected', {
        ip: req.ip,
    ...
    if (decision.isDenied() && decision.reason.isShield()) {
      logger.warn('Request blocked as shield detected', {
        ip: req.ip,
        userAgent: req.get('User-Agent'),
        path: req.path,
        method: req.method,
